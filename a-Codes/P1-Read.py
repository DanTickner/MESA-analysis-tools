...
output_dir = "aaa_Data/20210819_1" # Where to save folders. Leave as empty string to save in current directory.
models = [ "LOGS_0p69D" ] # [ f for f in os.listdir(input_dir) if f[:6]=="LOGS_Z" ] #[ "LOGS_0p" + str(i) for i in range(60,96) ]
...
def read_file( model_name ):
	...
	for n, name in enumerate( col_names ):
		if name[:8] == "surface_" and name[8:] in isotopes:
			...
			if not element in elements_h[0]:
				[ elements_h[i].append( val ) for i, val in enumerate([ element, f"M[{element.title()}]", f"N[{element.title()}]", f"log(N_abs[{element.title()}])" ]) ]

	history_headers = col_names + elements_h[1] + elements_h[2] + ["log(N_tot)"] + elements_h[3] +  calculated_col_names_h # Headers for history.csv.
	...
	for row in contents[6:]:
		...
		mass_fractions = []
		N_abs = [] # Extrapolated total number of atoms of each element, under the incorrect assumption that the surface abundances represent the entire star. Inaccurate but good for comparing between elements or stars.
		for el in elements_h[0]:
			...
			N_abs.append( element_mass_fraction / AW_weights[AW_names.index(el)] * star_mass * 1.99e30 / 1.67e-27 )
		values += mass_fractions
		N_tot = sum( N_abs ) # Total number of atoms in the star.
		values += [ N / N_tot for N in N_abs ] # Number fractions.
		values.append( np.log10( N_tot ) )# log total number of atoms in the star.
		values += [ np.log10( N ) for N in N_abs ] # log number of atoms of each element.
	...
	if do_profiles:
		...
		for p in profile_numbers:
			...
			if p == min( profile_numbers ):
				...
				elements_p = [ [], [], [] ] # element, col_name ( Rho[H] Rho[He] etc ), col_name ( log(N_abs[H]) log(N_abs[He] etc )
				for n, name in enumerate( p_headers ):
					if name in isotopes:
						...
						if not element in elements_p[0]:
							[ elements_p[i].append( val ) for i, val in enumerate([ element, f"Rho[{element.title()}]", f"log(N_abs[{element.title()}])" ]) ]
				# Now we can add headers row to both CSVs.
				[ w.writerow( [ "profile_number", "model_number" ] + p_headers + calculated_col_names_p + elements_p[1] + ["log(N_tot)" ] + elements_p[2] ) for w in [ p_writer, psurf_writer ] ]
			...
			for row in p_contents[6:]:
				...
				# Partial density of each element, units g/cm^3, and log total number of atoms in the zone.
				N_abs = [] # Number of atoms of each element in the zone.
				for el in elements_p[0]:
					element_mass_fraction = sum([ values[n] for i,n in enumerate(species_p[2]) if species_p[1][i] == el ])
					N_abs.append( element_mass_fraction * dM )
					values.append( element_mass_fraction * dM  / V_zone )
				N_tot = sum( N_abs ) # Total number of atoms in the zone.
				values.append( np.log10( N_tot) )
				values += [ np.log10( N ) for N in N_abs ]
