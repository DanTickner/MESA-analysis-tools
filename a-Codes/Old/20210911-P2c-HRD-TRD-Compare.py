# Reason archived: File structure has been simplified. Redundant "/CSV" folder has been removed.
...
model_letters = [ "20210809_B/LOGS_06", "20210809_D/LOGS_06", "20210809_R/LOGS_06", "20210809_RD/LOGS_06" ] #[ "0p" + str(i) for i in range(60,96) ] #[ "B", "D", "R", "RD" ]
folder_output = "Image_Backup/20210910_report" # Code will automatically add parent_dir to this. Just name the folder.
...
add_to_title_text = r""
custom_labels = [ "M=0." + str(ml)[-2:] for ml in model_letters ] # [ "Standard", "Diffusion", "Rotation", "Rotation and diffusion" ]
...
for  i_m, m in enumerate( model_letters ):
	...
	folders_csv.append( the_folder + "/CSV" )
...
for i in range(len( model_letters )):
	...
	ax.plot( log_Teff[i], log_L[i], color=(col,col,col), ls="-", label=labels[i], zorder=len(model_letters)-i )
...
for param, ylabel, lst in zip( ["Teff","L"], [r"$\log(T_{\rm{eff}})$",r"$\log(L/L_\odot)$"], [log_Teff,log_L] ):
	...
	for i in range(len( model_letters )):
		...
		ax2.semilogx( star_age[i], lst[i], color=(col,col,col), ls="-", label=labels[i], zorder=len(model_letters)-i )
...
for param, ylabel, lst in zip( ["Teff","L"], [r"$\log(T_{\rm{eff}})$",r"$\log(L/L_\odot)$"], [log_Teff,log_L] ):
	...
	for i in range(len( model_letters )):
		...
		ax2b.plot( model_number[i], lst[i], color=(col,col,col), ls="-", label=labels[i], zorder=len(model_letters)-i )
...
for i in range(len( model_letters )):
	...
	ax3.plot( log_Rhoc[i], log_Tc[i], color=(col,col,col), ls="-", label=labels[i], zorder=len(model_letters)-i )
...
for param, ylabel, lst in zip( ["Tc","Rhoc"], [r"$\log(T_{\rm{c}})$",r"$\log(\rho_{\rm{c}})$"], [log_Tc,log_Rhoc] ):
	...
	for i in range(len( model_letters )):
		...
		ax4.semilogx( star_age[i], lst[i], color=(col,col,col), ls="-", label=labels[i], zorder=len(model_letters)-i )
...
for param, ylabel, lst in zip( ["Tc","Rhoc"], [r"$\log(T_{\rm{c}})$",r"$\log(\rho_{\rm{c}})$"], [log_Tc,log_Rhoc] ):
	...
	for i in range(len( model_letters )):
		...
		ax4b.plot( model_number[i], lst[i], color=(col,col,col), ls="-", label=labels[i], zorder=len(model_letters)-i )
