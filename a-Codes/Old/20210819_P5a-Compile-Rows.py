# Reason archived: ignore_past_MSTO snippet was not strict enough. MSTO is a finite age range, not the single point of max Teff. Further, the enabling condition wasn't actually checked so it would ALWAYS ignore past the MSTO.
import os
import csv
import numpy as np

#----- Define variables -----
parent_dirs = [ "aaa_Data/20210817" ] # Each of these folders is expected to contain all members of csv_folders as subdirectories.
csv_folders = [ "Z=1d-4" ]
...
for parent in parent_dirs:
	...
	for subfolder in csv_folders:
		...
		if   condition == "final":
			...
			if max(log_Teff) == log_Teff[-1]:
				print( f"Ignoring model: Teff never decreases so MSTO likely not reached.\tlogTeff:{log_Teff[-1]}." )
				continue

		elif condition == "age":
			star_age = [ float(row[age_col]) for row in data[1:] ]
			log_Teff = [ float(row[Teff_col]) for row in data[1:] ]
			n_maxTeff = log_Teff.index(max( log_Teff )) + 1 # Offset by 1 due to headers.
