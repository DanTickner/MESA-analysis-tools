# Reason archived: Changing naming convention for mod and photos to allow more freedom.
...
#----- Define varibales -----
template_filename_inlist = "Templates_and_data/template_inlist_R"		# Name of template inlist.
template_filename_rn = "Templates_and_data/template_rn"				# Name of template rn file.
output_folder = "inlists/20210809a_initial_omega"	# Where to save inlists.
M_min = 0.68 #0.6	 #0.66		# Minimum stellar mass (inclusive).
M_max = 0.68 # 0.75 for z=1d-4, 0.83 safe to z=1d-2	# Maximum stellar mass (inclusive).
M_step = 0.01 #0.01 # 0.001		# Mass increment.

m_Li = 6.941				# Atomic mass in u
m_H = 1.00794				# Atomic mass in u
R_H = 2.3e-5				# Primodial abundance ratio 2H / 1H.
R_He = 3e-4				# Primodial abundance ratio 3He / 4He.
...
#----- Read template and replace metallicity placeholder -----
template_inlist = open( template_filename_inlist, "r" ).read().replace( "$METALLICITY$", Z_text ).replace("$H_HE_FRACS$",H_He_fracs).replace("$ZFRACS$",zfracs)
...
for M in M_range:
	...
	this_inlist = copy.copy( template_inlist ).replace( "$MASS$", M_string ).replace( "$MASS_STRING$", M_filename )
	
	#--- Save inlist and print filename to screen ---
	open( output_folder + "/" + output_filename, "w", newline="" ).write( this_inlist )
	print( output_filename )
	
	#--- Append to rn commands ---
	do_one_list.append( f"do_one z_Inlists/inlist_Z={Z_text}_M={M_filename} z_Mods/mod_Z={Z_text}_M={M_filename}.mod z_Logs/LOGS_Z={Z_text}_M={M_filename}" )
...
print( "\n1) GUI method: Right-click on ''rn'', go to ''Permissions'' tab and place a tick on ''Allow this file to run as a program''. This works on Linux but unfortunately I can't find that option on Windows and don't own a Mac." )
