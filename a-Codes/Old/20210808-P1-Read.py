# Reason archived: My initial calculation for partial density (20210806b) was on the right lines. Proper calculation added.
...
output_dir = "aaa_Data/20210806b" # Where to save folders. Leave as empty string to save in current directory.
...
calculated_col_names_p = [ ] # Must be in same order as calculations carried out in this code below. Currently empty.
	...
	if do_profiles:
		...
		for p in profile_numbers:
			...
			for row in p_contents[6:]:
				values = [ float(r) for r in row ]
				Rho = 10**( values[ p_headers.index( "logRho" ) ] ) # Density of zone in g/cm3. Need logRho for T-Rho diagram, so no point saving Rho separately when we can easily calculate it.

				#- Add calculated columns here -

				# Partial density of each element, units g/cm3
				for el in elements_p[0]:
					sum_isotope_masses = sum([ values[n] for i,n in enumerate(species_p[2]) if species_p[1][i] == el ])
					values.append( sum_isotope_masses * Rho )
